...
import { ArrowLeft, ArrowDownToLine, ArrowUpFromLine, Package, Calendar, User, Edit, Trash2, MoreVertical } from 'lucide-react';
import { StockItem, StockTransaction } from '../App';
...
interface ItemDetailProps {
  ...
  onEditTransaction: (transactionId: string, quantity: number, personName?: string, notes?: string) => void;
  onDeleteTransaction: (transactionId: string) => void;
}

export function ItemDetail({ item, transactions, onBack, onStockIn, onStockOut, onEditTransaction, onDeleteTransaction }: ItemDetailProps) {
                      ...
                      <div className="flex-1">
                        ...
                        {transaction.personName && (
                          <div className="flex items-center gap-1 text-sm text-gray-700 mt-2">
                            ...
                            <span>Released to: <span className="font-medium">{transaction.personName}</span></span>
                          </div>
                        )}
                        {transaction.notes && (
      ...
      <AlertDialog open={!!deletingTransactionId} onOpenChange={(open) => !open && setDeletingTransactionId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this transaction? This will adjust the current stock level and cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
